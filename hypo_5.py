...
def generate_hypotheses(tracks, reports):
    ...
    # Generate all possible pairings of tracks and reports, indexed from 1
    for hypothesis in np.ndindex(*(num_reports for _ in range(num_tracks))):
        hypotheses.append(tuple(h + 1 for h in hypothesis))

    print("\nGenerated Hypotheses:")
    for hypothesis in hypotheses:
        hyp_details = []
        for track_idx, report_idx in enumerate(hypothesis):
            hyp_details.append((track_idx + 1, report_idx, reports[report_idx - 1].tolist()))
        print(hyp_details)
...
for track_idxs, report_idxs in clusters:
    ...
    for hypothesis, probability in zip(hypotheses, probabilities):
        hyp_details = []
        for track_idx, report_idx in enumerate(hypothesis):
            hyp_details.append((track_idxs[track_idx] + 1, report_idxs[report_idx - 1] + 1, cluster_reports[report_idx - 1].tolist()))
